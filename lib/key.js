...
function find (objs, fullKey, all = false) {
  ...
  for (const obj of objs) {
    ...
    for (let index of indice) {
      ...
      } else if (['t', 'f'].includes(str[index])) { // boolean
        !arrIndice.length && arrOut.push(str[index] === 't')
      } else if (['n'].includes(str[index])) { // null
        !arrIndice.length && arrOut.push(null)
      } else if (!isNaN(Number(str[index]))) { // number
        !arrIndice.length && arrOut.push(findNumber(str, index))
      } else if (['"'].includes(str[index])) { // string
        !arrIndice.length && arrOut.push(findString(str, index))
      }
    }
  }
...
function findObjArr (str = '', index = 0, arrIndice = []) {
  let crlBrkt = 0
  let sqrBrkt = 0
